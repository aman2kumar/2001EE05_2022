...
class FirstScreen(tk.Tk):
    def __init__(self):
        ...
        self.x_co = int((screen_width / 2) - (1024 / 2))
        self.y_co = int((screen_height / 2) - (768 / 2)) - 80
        self.geometry(f"1024x768+{self.x_co}+{self.y_co}")
        ...
        background = Image.open("images/login_bg.png")
        background = background.resize((1026, 768), Image.ANTIALIAS)
        ...
        tk.Label(self.first_frame, image=background).place(x=-2, y=0)

        self.profile_label = tk.Label(self.first_frame, bg="white")
        self.profile_label.place(x=180, y=150, width=150, height=140)

        upload_button = tk.Button(self.first_frame, image=upload_image, compound="left", text="Upload Image",
                                  cursor="hand2", font="lucida", padx=2, command=self.add_photo)
        upload_button.place(x=175, y=320)

        # self.username = tk.Label(self.first_frame, text="Username", font="lucida 12 bold", bg="grey")
        # self.username.place(x=80, y=150)

        self.username_entry = tk.Entry(self.first_frame, font="lucida 12 bold", width=26, highlightcolor="blue", highlightthickness=1)
        self.username_entry.place(x=590, y=204, height=52)
        self.username_entry.focus_set()

        self.useremail_entry = tk.Entry(self.first_frame, font="lucida 12 bold", width=26, highlightcolor="blue", highlightthickness=1)
        self.useremail_entry.place(x=590, y=303, height=52)
        self.useremail_entry.focus_set()

        self.userroll_entry = tk.Entry(self.first_frame, font="lucida 12 bold", width=26, highlightcolor="blue", highlightthickness=1)
        self.userroll_entry.place(x=590, y=404, height=52)
        self.userroll_entry.focus_set()

        self.userpass_entry = tk.Entry(self.first_frame, font="lucida 12 bold", width=26, highlightcolor="blue", highlightthickness=1)
        self.userpass_entry.place(x=590, y=501, height=52)
        self.userpass_entry.focus_set()
        ...
        submit_button.place(x=640, y=584, width=190, height=52)
...
class ChatScreen(tk.Canvas):
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id):
        ...
        tk.Label(self, text="   ", font="lucida 15 bold", bg="black").place(x=4, y=29)

        tk.Label(self, text="CHAT ROOM", font="lucida 15 bold", padx=20, fg="white",
                 bg="black", anchor="w", justify="left").place(x=88, y=29, relwidth=1)
        ...
        self.canvas = tk.Canvas(container, bg="#d9d5d4")
        self.scrollable_frame = tk.Frame(self.canvas, bg="#097969")
        ...
        send_button = tk.Button(self, text="SEND", fg="white", font="lucida 11 bold", bg="black", padx=10,
                                relief="solid", bd=2, command=self.sent_message_format)
        send_button.place(x=400, y=685)
        ...
        m_label = tk.Label(m_frame, wraplength=250, text=f'''Welcome "{self.parent.user}"''', font="lucida 10 bold", bg="orange")
        m_label.pack(fill="x")
    ...
    def received_message_format(self, data):
        ...
        m_frame = tk.Frame(self.scrollable_frame, bg="#097969")

        m_frame.columnconfigure(1, weight=1)

        t_label = tk.Label(m_frame, bg="#097969",fg="white", text=datetime.now().strftime('%H:%M'), font="lucida 7 bold",
                        justify="left", anchor="w")
        t_label.grid(row=0, column=1, padx=2, sticky="w")

        m_label = tk.Label(m_frame, wraplength=250,fg="black", bg="white", text=message, font="lucida 9 bold", justify="left",
                        anchor="w")
        m_label.grid(row=1, column=1, padx=2, pady=2, sticky="w")

        i_label = tk.Label(m_frame, bg="#097969", image=im)
        i_label.image = im
    ...
    def sent_message_format(self, event=None):
        ...
        if message:
            ...
            t_label = tk.Label(m_frame, bg="#097969", fg="white", text=datetime.now().strftime('%H:%M'),
                               font="lucida 7 bold", justify="right", anchor="e")
            t_label.grid(row=0, column=0, padx=2, sticky="e")

            m_label = tk.Label(m_frame, wraplength=250, text=message, fg="black", bg="#ECFFDC",
                               font="lucida 9 bold", justify="left",
                               anchor="e")
            m_label.grid(row=1, column=0, padx=2, pady=2, sticky="e")

            i_label = tk.Label(m_frame, bg="#097969", image=self.user_image)
    ...
    def notification_format(self, data):
        ...
        t_label = tk.Label(m_frame, fg="black", bg="#d9d5d4", text=datetime.now().strftime('%H:%M'), font="lucida 9 bold")
        t_label.pack()

        m_label = tk.Label(m_frame, wraplength=250, text = message, font="lucida 10 bold", justify="left", bg=color_code)
        m_label.pack(fill = "x")
